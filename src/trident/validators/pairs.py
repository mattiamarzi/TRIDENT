...
pairs.py
══════════
Two–star (country–pair) validator under a bipartite null model.
...
computes p-values for the number of common neighbors (V-motifs) of
each country pair (i, j) under several approximations:
...
Crucially, we enumerate **all** pairs i<j, even if their observed
count is zero, to match classical multiple-testing setups (FDR, etc.).
...
def _pair_counts(adj: list[set[int]], include_absent: bool = True) -> dict[int, dict[int, int]]:
    """
    Observed V-motif multiplicities for all country pairs.

    Returns
    -------
    counts : dict
        Nested dict counts[i][j] = |N(i) ∩ N(j)| for all i < j.
        If include_absent is True (default) we include zero counts too,
        so **every** pair obtains a p-value.
    """
...
_counts = None   # observed counts dict
...
def _worker(task: tuple[int, int]):
    """
    Compute one p-value for the pair (i, j).
    Uses globals set by _init_globals.
    """
    i, j = task
    v_obs = _counts[i][j]
    if v_obs == 0:
        return i, j, 1.0        # P{S ≥ 0} = 1, skip all computations
    probs = _Pmat[i] * _Pmat[j]     # q_α  =  P_{iα} * P_{jα}
    mu    = probs.sum()

    if _method == "poisson":
        # Upper tail of Poisson(μ)
        return i, j, float(poisson.sf(v_obs - 1, mu))
    ...
    if _method == "normal":
        ...
        if var <= 0.0:
            ...
            return i, j, (1.0 if v_obs <= mu else 0.0)
        z = (v_obs + 0.5 - mu) / math.sqrt(var)
        return i, j, float(norm.sf(z))
    ...
    if _method == "poibin":
        ...
        return i, j, float(PoiBin(probs).pval(v_obs))
...
# Public validator
# =============================================================================
class TwoStarValidator:
    """
    Validate country pairs (two-stars / V-motifs with size 2) against a null model.
    ...
    include_absent : bool, default True
        If True, assign a p-value to every pair i<j (including those
    ...
    def __init__(self,
        ...
        self.counts = _pair_counts(self.adj, include_absent=include_absent)

    # .........................................................................
    def compute_pvals(self, method: str = "poibin") -> dict[int, dict[int, float]]:
        """
        Compute p-values for all pairs i<j under the requested method.

        Returns
        -------
        pvals : dict
            Nested dict pvals[i][j] = p-value for i<j.
        """
        tasks = [(i, j) for i in self.counts for j in self.counts[i]]
        ...
        with ProcessPoolExecutor(max_workers=self.n_jobs,
            ...
            # tune chunksize: larger = fewer IPC calls; 5_000–20_000 is usually good
            chunk = 10_000
            iterator = ex.map(_worker, tasks, chunksize=chunk)
            if self.progress:
                iterator = tqdm(iterator, total=len(tasks))
            for i, j, p in iterator:
                out.setdefault(i, {})[j] = float(p)
        return out
    ...
    def validate(self,
                 pvals_dict: dict[int, dict[int, float]],
                 alpha: float = 0.05,
                 method: str = "fdr",
                 names: list[str] | None = None) -> tuple[pd.DataFrame, float]:
        """
        Validate pairs at level α with the chosen multiple-testing rule.

        Returns
        -------
        df : DataFrame
            Columns: ('i', 'j', 'p') (indices or names).
        thr : float
            The p-value threshold actually used.
        """
        rec: list[tuple[int, int, float]] = []
        for i in pvals_dict:
            for j, p in pvals_dict[i].items():
                if p is None or not np.isfinite(p):
                    continue
                rec.append((i, j, float(p)))

        if not rec:
            return pd.DataFrame(columns=["i", "j", "p"]), 0.0

        flat = np.fromiter((r[-1] for r in rec), dtype=float)
        thr  = self._threshold(flat, alpha, method)
        df   = pd.DataFrame(rec, columns=["i", "j", "p"])
        df   = df.query("p <= @thr").reset_index(drop=True)

        if names is not None:
            rep = {idx: nm for idx, nm in enumerate(names)}
            df[["i", "j"]] = df[["i", "j"]].replace(rep)
